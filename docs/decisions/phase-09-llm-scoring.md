...
- Backend знает `LiteLLMProvider`, и plumbing/readiness слой для него реализован.
...
- Backend-driven secured inference через текущий `LiteLLMProvider` не считался auth-verified результатом этой фазы.
